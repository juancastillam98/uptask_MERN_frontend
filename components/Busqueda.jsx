...
export default function Busqueda(){
    ...
    //proyectos filtrados va a devolver un listado con los proyectos que contenga lo que hemos introducido en la búsqueda
    const proyectosFiltrados = busqueda ==="" ? [ ]:proyectos.filter(proyecto => proyecto.nombre.toLowerCase().includes(busqueda.toLowerCase()))

    return (
        /*El afterLeave es un hook para mandar a ejecutar algo cuando se cierre el compoente*/
                      ...
                      {proyectosFiltrados.length > 0 && (
                            <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                                {proyectosFiltrados.map(proyecto => {
                                    console.log('ID:', proyecto.nombre);
                                    return (
                                        <Combobox.Option
                                            key={proyecto._id}
                                            value={proyecto}
                                            className={({ active }) => classNames("cursor-default select-none px-4", active && "bg-sky-600 text-white")}
                                        >
                                            {proyecto.nombre}
                                        </Combobox.Option>
                                    );
